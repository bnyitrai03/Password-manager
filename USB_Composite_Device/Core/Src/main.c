...
int main(void)
{
  ...
  USBD_CDC_ACM_HandleTypeDef *h = &CDC_ACM_Class_Data[0];
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  HAL_Delay(1000);
		...
		USBD_HID_Keyboard_SendReport(&hUsbDevice, &keyBoardHIDsub, sizeof(keyBoardHIDsub));
		...
		USBD_HID_Keyboard_SendReport(&hUsbDevice, &keyBoardHIDsub, sizeof(keyBoardHIDsub));
		HAL_Delay(5000); 	       // Repeat this task on every 5 second*/
